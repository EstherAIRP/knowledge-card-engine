...
前 10 段是最終呈現結構，不是研究計畫或閱讀來源的順序。Agent 應先依本輪最終證據形成整體理解，再依 [Knowledge Card 知識編輯提示契約](../prompts/KNOWLEDGE_EDITOR.md) 映射到各段；不得把 research question、structured findings 或 README 章節逐項改寫成正文。

各段的內容責任如下：

- 「一句話介紹」說清楚資源本質，不複製宣傳語。
- 「它解決什麼問題」說明具體問題與使用情境。
- 「核心概念」優先保存核心抽象、模型或設計思想。
- 「架構與技術」說明主要元件責任、邊界與關鍵流程，不以技術棧清單代替架構。
- 「主要功能」保留有助於理解用途的主要能力，不必搬運來源完整功能目錄。
- 「技術亮點」說明有辨識度的機制或設計決策，以及其價值。
- 「限制與風險」描述專案特有的限制、取捨與實際影響；來源不支持的部分不得用泛用警語補滿。
- 「與你的相關性」只能使用 Workspace 明確允許的分析背景。
- 「建議怎麼使用」應由前述問題、機制、限制與適用情境推導。
- 「與其他收藏的關聯」應表達互補、替代、延伸、前置技術、實作案例或相反設計等實質關係，不以共同標籤取代關係判斷。

各段不要求固定字數或固定項目數；來源簡單時可保持簡潔，證據不足時應誠實表達未知，不得為了段落完整而臆造內容。
