...
- Card detail browser route 使用 `/knowledge/<stable-id>`。點開 Card 後以 History API 寫入 permalink；重新整理或直接開啟該網址時，server 只回公開 shell，browser 完成 session / Workspace authorization 後才由 `/api/cards/:id` 取得正文並恢復同一張 Card。Browser back / forward 由 `popstate` 依目前 URL 在 Card detail 與首頁之間切換；route 本身不繞過 private API authorization。
- Card detail：先以寬版 metadata surface 顯示來源、狀態、Navigation Category、Action、Relevance、Tag 與日期；Markdown 正文維持較窄 reading width；桌機右側提供 sticky「文章目錄」，較窄 viewport 則把「文章目錄」整合進主 sticky header，與 Cards / Search / Graph 共用同一列，點擊後在 header 下方展開同一份章節清單，不再額外保留第二條 sticky local-nav。目錄依序納入正文 H2／H3，以及實際存在的 Concept Neighborhood、Related Knowledge 頂層區段，點擊可捲動定位且會隨閱讀位置標示目前章節；Concept Neighborhood 與 Related Knowledge 在正文後維持寬版內容區，以卡片網格呈現。Concept 卡會切換至 Graph 並以該 Concept 作為查詢；Related Knowledge 顯示鄰卡摘要與目前 relation artifact 可提供的 score／evidence／manual note。
