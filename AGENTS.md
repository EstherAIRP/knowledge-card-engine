...
- Remote Ingest handoff 只能在專用 `chore/ingest-*` Workspace 分支執行；`state/ingestion/` 的 request、accepted evidence、Threads semantic handoff、GitHub research evidence、必要時的 GitHub research plan 與 analysis 都是暫存交換資料，正式 apply 成功後必須移除，不得進入 Workspace `main`。GitHub prepare 只固定 revision、建立 bounded discovery 與空的 round-0 research state；Agent 必須先提交 digest-bound research plan 與 selected paths，runner 才擷取第一輪 validated research evidence。第一輪 bundle 形成後，Agent 可提交 analysis，或在剩餘 budget 內再提交一次 digest-bound research plan。Selected path 不以 discovery candidate set 作為 allowlist，但必須是同一 pinned revision 中可驗證的安全 repository-relative 文字檔，且不得位於 Engine 排除目錄。Agent 的 request / judgement / research-plan / analysis 提交仍必須遵守單一輸入檔與 runner-parent lineage 守門，不得修改 runner-owned evidence state。GitHub research evidence handoff 可暫存 selected source text 供分析，但正式 research state 只能保留 compact provenance。
