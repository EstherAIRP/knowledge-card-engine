...
test('private-site shell exposes the Knowledge Radar presentation without embedding private content', () => {
  ...
  assert.match(html, /knowledge-reading/u);
  assert.match(html, /Concept Neighborhood/u);
  ...
  assert.match(html, /graph-controls__head/u);
  assert.match(html, /SEMANTIC KNOWLEDGE MAP/u);
